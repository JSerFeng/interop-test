...
let byTestCaseMd = fs.readFileSync(
  path.resolve(root, "by-test-case.md"),
  "utf-8"
);
byTestCaseMd = byTestCaseMd.replace(
  /<!-- module results -->\n\n(## .+\n|\n|\|.+\n)+/,
  `<!-- module results -->\n\n${outputModule}`
);
fs.writeFileSync(path.resolve(root, "by-test-case.md"), byTestCaseMd);

let bySyntaxMd = fs.readFileSync(path.resolve(root, "by-syntax.md"), "utf-8");
bySyntaxMd = bySyntaxMd.replace(
  /<!-- syntax results -->\n\n(## .+\n|\n|\|.+\n)+/,
  `<!-- syntax results -->\n\n${outputSyntax}`
);
fs.writeFileSync(path.resolve(root, "by-syntax.md"), bySyntaxMd);
...
let diffsMd = fs.readFileSync(path.resolve(root, "diffs.md"), "utf-8");
for (const diff of diffs) {
  ...
  diffsMd = diffsMd.replace(
    new RegExp(`<!-- diff-${name} results -->\\n\\n(\\n|\\|.+\\n)+`),
    `<!-- diff-${name} results -->\n\n${output}`
  );
}
fs.writeFileSync(path.resolve(root, "diffs.md"), diffsMd);
